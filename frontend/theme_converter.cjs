...
const postProcessRegexes = [
    { regex: /\.dashboard-container\s*\{[^}]+\}/g, fn: match => match.replace(/var\(--charcoal\)/g, 'var(--black)') },
    { regex: /\.pm-container\s*\{[^}]+\}/g, fn: match => match.replace(/var\(--charcoal\)/g, 'var(--black)') },
    { regex: /\.receptionist-dashboard\s*\{[^}]+\}/g, fn: match => match.replace(/var\(--charcoal\)/g, 'var(--black)') },
    { regex: /\.client-details-page\s*\{[^}]+\}/g, fn: match => match.replace(/var\(--charcoal\)/g, 'var(--black)') },
    { regex: /\.supervisor-dashboard\s*\{[^}]+\}/g, fn: match => match.replace(/var\(--charcoal\)/g, 'var(--black)') },
];
...
files.forEach(file => {
    ...
    postProcessRegexes.forEach(({ regex, fn }) => {
